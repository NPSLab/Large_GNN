...
class IGL260MDataset(object):
    ...
    def paper_feat(self) -> np.ndarray:
        path = '/tmp/job.300580/datasets/large/processed/paper/node_feat.npy'
    ...
    def paper_label(self) -> np.ndarray:
        if self.num_classes == 19:
            path = '/tmp/job.300580/datasets/large/processed/paper/node_label_19.npy'
            print(f"19 version used")
        else:
            path = '/tmp/job.300580/datasets/large/processed/paper/node_label_2K.npy'
    ...
    def paper_edge(self) -> np.ndarray:
        ...
        path = '/tmp/job.300580/datasets/large/processed/paper__cites__paper/edge_index.npy'
        return np.load(path, mmap_mode='r')
        # if self.in_memory:
        #     return np.load(path)
        # else:
...
class IGL260M(DGLDataset):
    ...
    def process(self):
        dataset = IGL260MDataset(root='/tmp/job.300580/datasets', size='large', in_memory=0, classes=19)
        ...
        self.graph_g = Data(x=node_features, y=node_labels, adj_t=adj_t, edge_index = node_edges)
